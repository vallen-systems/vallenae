...
@jit(f8(f8[:], f8, f8[:, :], f8[:]))
def lucy_error_fun(test_pos: np.ndarray,
                   speed: float,
    ...
    for i in range(m):
        theo_delta_dists[i] = theo_dists[i+1] - \
                              theo_dists[0]
...
def main():
    ...
    pridb_path = os.path.join(this_file_path,
                              'steel_plate/sample.pridb')
    ...
    tree = ElementTree.parse(
        os.path.join(this_file_path,
                     'steel_plate/sample.vaex'))
    root = tree.getroot()
    ...
    for i, elem in enumerate(xml_chans):
        unordered_sens_poss[i, :] = [float(elem.get('X')),
                                     float(elem.get('Y'))]
    ...
    # Heatmap computation:
    # -------------------
    lucy_instance_2args = (lambda x, y: lucy_error_fun(np.array([x, y]),
                                                       speed_m_per_s,
                                                       ordered_sens_poss,
                                                       delta_ts))

    x_range = arange(location_search_bounds[0][0],
                     location_search_bounds[0][1],
                     grid_delta)
    y_range = x_range
    ...
    location_result = \
        differential_evolution(lucy_instance_single_arg,
    ...
    print('Runtime for 1 call to differential_evolution(): ',
          end - start)
