...
def read_tra(abs_file_name: str, from_: int, to_: int) -> pd.DataFrame:
    """
...
def extract_wave(
    ...
    if data_format not in {PLAIN_SPECIFIER, FLAC_SPECIFIER}:
        raise TypeError("Unknown BLOB format. data_format = " + str(data_format))

    do_use_flac = data_format == FLAC_SPECIFIER

    return _extract_wave(
        pd_tra_frame, tra_idx, do_use_flac, do_convert2v, do_create_time_vector
    )


def read_labels(abs_file_name: str) -> pd.DataFrame:
    """
    ...
    """
    query = "SELECT SetID, Number, Data " + "FROM view_ae_markers"
    data_frame = None
...
def _read_db(
    abs_file_name: str, tbl_name: str, indx: str, from_: int, to_: int
) -> pd.DataFrame:
    """
    ...
    if os.path.exists(abs_file_name):
        with ctxlib.closing(sqlite3.connect(abs_file_name)) as db_conn:
            data_frame = pd.read_sql_query(query, db_conn, params=binding_params)
    if data_frame is not None:
        data_frame = data_frame.set_index(indx)
    return data_frame


def _get_sr_pt_smpls_2mv(
    pd_tra_frame: pd.DataFrame, tra_idx: int
...
def _create_time_vector(
    nb_pretrigs: int, nb_samples: int, sample_rate: int
) -> np.ndarray:
    """
...
def _extract_wave(
    ...
    """
    sr, nb_pretrigs, nb_smpls, to_mv_factor = _get_sr_pt_smpls_2mv(
        pd_tra_frame, tra_idx
    )
