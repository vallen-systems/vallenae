...
def energy_ratio(arr: np.ndarray, win_len: int = 100) -> Tuple[np.ndarray, float, int]:
    safetye_eps = 1e-6
    n = len(arr)
    er_vals = np.zeros(n)
    l_sumsq = 0.0
...
if __name__ == "__main__":
    ...
    tra_frame = vae.read_tra(
        os.path.join(THIS_FILE_PATH, "steel_plate/sample_plain.tradb"), 1, MAX_TRA_IDX
    )
    ...
    ax1.plot(CONVERT_SECS_2_USECS * xs[0:END_SAMPLE], ys[0:END_SAMPLE], color="g")
    ax1.tick_params(axis="y", labelcolor="g")

    ax2 = ax1.twinx()
    ax2.set_ylabel("Energy Ratio [/]", color="r")
    ax2.plot(CONVERT_SECS_2_USECS * xs[0:END_SAMPLE], er_arr[0:END_SAMPLE], color="r")
